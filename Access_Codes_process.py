...
def run_special_request(Credentials, Billing_ISBNs, VBIDs, quantities, Verba_Schools, Catalogs, Publishers, process_todo):
    ...
    # Start run for
    case = 0
    for Billing_ISBN, VBID, Total_quantity, Verba_School, Catalog, Publisher in zip(Billing_ISBNs, VBIDs, quantities, Verba_Schools,
                                                                              Catalogs, Publishers):
        case += 1
        print('\nCase {} of {}\n{} - {} - {}'.format(case, len(Billing_ISBNs), Verba_School, Catalog, Billing_ISBN))
        File_imported = False
        ...
        if Publisher == 'eCampus Content Holding':
            ...
            if Total_quantity < Available_codes:
                ...
                # Move access codes file to history folder
                Functions.move_csv_file(Error=Error, Check_file=Ruby_run_error, School=Verba_School, Catalog=Catalog,
                                        save_path=save_path, access_codes_file=access_codes_file, process=process_todo)

        # Add if this title run successfully or not to report
        Report = Functions.append_to_report(Report=Report, File_imported=File_imported, Error=Error,
                                            Missing_codes=Missing_codes, School=Verba_School, Catalog=Catalog,
                                            Publisher=Publisher, Title='', Billing_ISBN=Billing_ISBN,
                                            quantity=quantity)
